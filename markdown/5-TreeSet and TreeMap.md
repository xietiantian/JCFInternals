...
下图里的情况1~6应该是原作者从代码中提炼出来的，我来加个文字版。整个调整过程是一个循环，每次循环都是针对一个节点x进行调整。循环结束后将根节点重新设为黑色，确保满足红黑树的规则2。p表示x的父节点，y表示x的叔节点，pp表示x的祖父节点。

情况1~3：p是pp左孩子，y是pp右孩子。
1. 红叔，也就是y是红色：将p、y设为黑色，pp为红色，x<--pp
2. 黑叔，且x是p的右孩子：x<--p，以新的x为中心左旋，x变成了p的左孩子，进入情况3
3. 黑叔，且x是p的左孩子：p设为黑色，pp设为红色，以pp为中心右旋
情况4~6：p是pp右孩子，y是pp左孩子。与上面对称，略。
